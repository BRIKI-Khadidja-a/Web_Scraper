...
def load_data():
    try:
        ...
        st.info("💡 Vérifiez votre connexion Supabase et que la table 'jobs' existe.")
        return pd.DataFrame()
...
if df.empty:
    ...
    st.info(
        "🔍 Aucune donnée à afficher. Veuillez d'abord scraper des jobs depuis la page Browse."
    )
    st.stop()
...
with col2:
    ...
    location_stats = (
        df.groupby("location").agg({"id": "count", "company": "nunique"}).reset_index()
    )
    ...
    st.dataframe(location_stats, hide_index=True, use_container_width=True, height=400)
...
with col1:
    ...
    source_stats = (
        df.groupby("source")
        .agg({"id": "count", "company": "nunique", "location": "nunique"})
        .reset_index()
    )
    ...
    st.dataframe(source_stats, hide_index=True, use_container_width=True)
...
with col2:
    ...
    selected_source = st.selectbox(
        "Select Source", options=sources, key="source_selector"
    )
...
with col2:
    ...
    recent_jobs = df.nlargest(10, "scraped_at")[
        ["company", "title", "source", "scraped_at"]
    ]
    recent_jobs["scraped_at"] = recent_jobs["scraped_at"].dt.strftime("%Y-%m-%d %H:%M")
    recent_jobs.columns = ["Company", "Title", "Source", "Scraped At"]

    st.dataframe(recent_jobs, hide_index=True, use_container_width=True, height=400)
...
with col1:
    ...
    completeness_df["Percentage"] = (
        completeness_df["Complete"] / completeness_df["Total"] * 100
    ).round(1)

    st.dataframe(
        completeness_df[["Field", "Percentage"]],
        hide_index=True,
...
st.markdown("---")
st.markdown(
    "**CyberScraper Analytics** - Real-time insights from Emploitic, France Travail & Wuzzuf"
)
